...
export default function LongApplicationForm({ jobId }: { jobId: string }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    alert(`✅ Application submitted successfully! (Job ID: ${jobId})`)
    router.push('/')
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-10 pt-8 border-t border-slate-200">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-6 pt-1">
              {['Yes', 'No', 'Yes, with limitations'].map(opt => (
                <label key={opt} className="flex items-center gap-2 text-sm"><input type="radio" name="remoteWork" value={opt} checked={formData.remoteWork === opt} onChange={handleChange} className="w-4 h-4" /> {opt}</label>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4 border-b-2 border-yellow-400 pb-2 inline-flex">
          <h3 className="text-lg font-bold text-slate-900">Background & Experience</h3>
        </div>
        <div className="space-y-4">
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Experience, coursework, volunteering, freelance work, or community involvement.</label><textarea name="experience" rows={3} value={formData.experience} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Describe one project you are proud of.</label><textarea name="proudProject" rows={3} value={formData.proudProject} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Software, technical, or creative skills.</label><textarea name="techSkills" rows={2} value={formData.techSkills} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Languages you can work in.</label><input type="text" name="languages" value={formData.languages} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none focus:border-yellow-400 text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Achievements, certifications, or communities.</label><textarea name="achievements" rows={2} value={formData.achievements} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
        </div>
      </div>
      ...
      <div className="mb-8 bg-slate-50 p-6 rounded-xl border border-slate-200">
        <div className="flex items-center gap-3 mb-4">
          <h3 className="text-lg font-bold text-slate-900">Motivation & Growth</h3>
        </div>
        <div className="space-y-4">
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Why do you want to join our team?</label><textarea name="motivationJoin" rows={3} value={formData.motivationJoin} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">What excites you about our preferred department?</label><textarea name="motivationExcites" rows={3} value={formData.motivationExcites} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">What value can you bring to your preferred department?</label><textarea name="motivationValue" rows={3} value={formData.motivationValue} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Biggest strength.</label><textarea name="biggestStrength" rows={2} value={formData.biggestStrength} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">Biggest development area.</label><textarea name="biggestGrowth" rows={2} value={formData.biggestGrowth} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">What are your career goals over the next two years?</label><textarea name="careerGoals" rows={3} value={formData.careerGoals} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
        </div>
      </div>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4 border-b-2 border-yellow-400 pb-2 inline-flex">
          <h3 className="text-lg font-bold text-slate-900">Scenario Questions</h3>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div><label className="block font-medium mb-1 text-sm text-slate-700">You realize you may miss a deadline. What do you communicate, and when?</label><textarea name="scenarioMissDeadline" rows={3} value={formData.scenarioMissDeadline} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">A teammate disagrees with your idea. How do you handle it?</label><textarea name="scenarioDisagree" rows={3} value={formData.scenarioDisagree} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">You are asked to do something you have never done. What is your first step?</label><textarea name="scenarioNewTask" rows={3} value={formData.scenarioNewTask} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
          <div><label className="block font-medium mb-1 text-sm text-slate-700">You notice a user-safety concern in a product or campaign. What would you do?</label><textarea name="scenarioSafety" rows={3} value={formData.scenarioSafety} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg outline-none font-inherit resize-y text-sm" /></div>
        </div>
      </div>
      ...
      <div className="border-t border-slate-200 pt-6">
        ...
        <button type="submit" className="w-full bg-slate-900 text-white py-4 rounded-full font-bold text-base hover:bg-slate-800 transition">Submit Application</button>
      </div>
    </form>
  )
}