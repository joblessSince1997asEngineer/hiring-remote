...
export default function CandidatesTable({ initialJobs }: { initialJobs: any[] }) {
  ...
  // Fetch candidates with 300ms debounce
  useEffect(() => {
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase">Assign to Job</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-400 text-sm">Searching...</td>
                </tr>
              ) : candidates.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-400 text-sm">No candidates found.</td>
                </tr>
              ) : (
                candidates.map((candidate) => (
                  <tr key={candidate.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-slate-600 text-sm">{candidate.primary_skill !== 'Not Set' ? candidate.primary_skill : '-'}</td>
                    <td className="p-4 text-slate-600 text-sm">{candidate.years_exp ? `${candidate.years_exp} yrs` : '-'}</td>
    ...
    <span className="text-slate-400">-</span>
                    ...
                    <td className="p-4">
                      <div className="flex gap-2">
                        <select
                          id={`job-${candidate.id}`}
                          className="p-2 border border-slate-300 rounded text-sm flex-1 min-w-[120px]"
                        >
                        ...
                        <button
                          ...
                          className="bg-black text-white px-4 py-2 rounded text-sm font-medium hover:bg-slate-800 disabled:opacity-50"
                        >
